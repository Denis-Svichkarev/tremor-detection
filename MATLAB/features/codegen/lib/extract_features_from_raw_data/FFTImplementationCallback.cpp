  ...
  namespace internal
  {
      ...
      &sintabinv, ::coder::array<creal_T, 1U> &y)
    {
      ...
      if ((n2blue != 1) && ((nfft & 1) == 0)) {
        ...
        for (k = 0; k < nInt2m1; k++) {
          nt_re_tmp = (nfft + k) - 1;
          y[k].re = wwc[nt_re_tmp].re * x[rt];
          y[k].im = wwc[nt_re_tmp].im * -x[rt];
          rt++;
        }
    ...
    // Return Type  : void
    //
    void FFTImplementationCallback::generate_twiddle_tables(int nRows, bool
    ...
    // Return Type  : void
    //
    void FFTImplementationCallback::get_algo_sizes(int nfft, bool useRadix2, int
      *n2blue, int *nRows)
    {
      *n2blue = 1;
      if (useRadix2) {
    ...
    void FFTImplementationCallback::r2br_r2dit_trig(const ::coder::array<double,
      1U> &x, int n1_unsigned, const ::coder::array<double, 2U> &costab, const ::
