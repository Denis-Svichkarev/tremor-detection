    ...
    for j = 0:timewindow_size:size(data, 2)
        if j + timewindow_size <= size(data, 2)
            ...
            % Frequency domain features
             ...
             data_features{1, index} = amp_X1;
             data_features{2, index} = W;
             ...
             data_features{8, index} = I_X1;
             data_features{9, index} = Q_X1;
             data_features{10, index} = SK_X1;
             data_features{11, index} = K_X1;
                     
             % Time domain features
                    
             data_features{12, index} = Q_T_X1;
             data_features{13, index} = M_T_X1;
             data_features{14, index} = S_T_X1;
             data_features{15, index} = M2_T_X1;
             data_features{16, index} = maxValue_T_X1;
             data_features{17, index} = minValue_T_X1;
             data_features{18, index} = I_T_X1;
